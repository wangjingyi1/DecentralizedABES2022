...
//授权用户属性
func (u *User) KeyGenByUser(gid string, attr string, d *DABE, hash hash.Hash) (*pbc.Element, *pbc.Element, *pbc.Element, error) {
	if u.ASKMap[attr] == nil {
		return nil, nil, nil, fmt.Errorf("don't have this attr, error when %s", attr)
	}
	Sik1 := d.CurveParam.GetG1FromStringHash(gid, hash).ThenPowZn(u.ASKMap[attr].Y)                          //H(GID)^y
	Sik2 := u.GAlpha                                                                                         //g^alpha
	key := d.CurveParam.GetG1FromStringHash(gid, sha256.New()).ThenPowZn(u.ASKMap[attr].Y).ThenMul(u.GAlpha) //H(GID)^y * g^alpha
	return key, Sik1, Sik2, nil
}
