...
func (d *DABE) Sign(hash hash.Hash, m string, gid string, uPolicy string, authorities map[string]Authority, SignKey1s map[string]*pbc.Element, SignKey2s map[string]*pbc.Element) (*Signature, error) {
	fmt.Println("DABS Signature start")

	hashn := sha256.New()
	hashM := d.CurveParam.GetZnFromStringHash(m, hashn)
	...
	sig1xs := make([]*pbc.Element, n, n)
	sig2xs := make([]*pbc.Element, n, n)
	upTemps := make([]*pbc.Element, n, n)
	downTemps := make([]*pbc.Element, n, n)
	hashMs := hashM.ThenMul(s)
	sig0 := d.EGG.NewFieldElement().PowZn(d.EGG, hashMs)

	attrs := make([]string, 0, 0)

	for key, _ := range SignKey1s {
	...
	for i := 0; i < n; i++ {
		...
		hashGid := d.CurveParam.GetG1FromStringHash(gid, hash)

		//H(GID)^r
		sig1x := d.G.NewFieldElement().PowZn(hashGid, r)

		//Ai*v
		AiV := policy.AccessStruct.LsssMatrixDotMulVector(i+1, v)
		//e(g,g)^(Ai*v)
		upTemp := d.EGG.NewFieldElement().PowZn(d.EGG, AiV)
		//Ai*w
		AiW := policy.AccessStruct.LsssMatrixDotMulVector(i+1, w)
		// g ^(Ai*w)
		gw := d.G.NewFieldElement().PowZn(d.G, AiW)
		//e(H(GID),g^w)
		temp1 := d.EGG.NewFieldElement().Pair(hashGid, gw)
		//e(g,g)^(Ai*v) * e(h(GID),g^w)
		upTemp.ThenMul(temp1)
		//e(g^(alpha_i),g)
		downTemp := d.EGG.NewFieldElement().Pair(SignKey2s[attrStr], d.G)
		//g^r
		gr := d.G.NewFieldElement().PowZn(d.G, r)
		//e(H(GID)^y,gr)
		temp2 := d.EGG.NewFieldElement().Pair(SignKey1s[attrStr], gr)
		fmt.Println("temp2signtemp2sign")
		fmt.Println(temp2)
		//e(g^(alpha_i),g)*e(H(GID)^y,gr)
		downTemp.ThenMul(temp2)
		sig2x := d.EGG.NewFieldElement().Div(upTemp, downTemp)

		sig1xs[i] = sig1x
		sig2xs[i] = sig2x
		upTemps[i] = upTemp
		downTemps[i] = downTemp
	}
	fmt.Println("DABS Signature success")
	return &Signature{
		Sig0:    sig0,
		Sig1xs:  sig1xs,
		Sig2xs:  sig2xs,
...
func (d *DABE) Verify(signature *Signature, authorities map[string]Authority) (bool, error) {
	fmt.Println("DABS Verify start")
	s := signature.s
	tempp := d.EGG.NewFieldElement().PowZn(d.EGG, s)
	fmt.Println("resresresresresres")
	fmt.Println(tempp)
	policy := new(Policy)
	d.growNewPolicy(signature.Policy, d.CurveParam.GetNewZn(), policy)
	n := len(policy.AccessStruct.LsssMatrix) - 1
	...
	hash := signature.Hash
	m := signature.M
	hashM := d.CurveParam.GetZnFromStringHash(m, hash)

	temp := d.CurveParam.Get1FromZn()
	temp = temp.ThenDiv(hashM)
	leftres := signature.Sig0.ThenPowZn(temp)
	rightres := d.CurveParam.Get1FromGT()
	tempres := d.CurveParam.Get1FromGT()
	for i := 0; i < n; i++ {
		if cxs[i+1] == nil {
			continue
		}
		...
		ttemp := authority.GetPK()
		//e(sig1x, g^y)
		ttemp1 := d.EGG.NewFieldElement().Pair(signature.Sig1xs[i], pk.Gy)
		fmt.Println("ttempverifyttempverify")
		fmt.Println(ttemp1)
		ttemp.ThenMul(ttemp1)
		ttemp.ThenMul(signature.Sig2xs[i])
		ttemp.ThenPowZn(cxs[i+1])
		rightres.ThenMul(ttemp)
		upTemp := signature.upTemps[i]
		upTemp.ThenPowZn(cxs[i+1])
		tempres.ThenMul(upTemp)
	}
	fmt.Println("left")
	fmt.Println(leftres)
	fmt.Println("right")
	...
	res := leftres.Equals(rightres)
	if res {
		fmt.Println("truetrue")
		return true, nil
	}
	return false, nil
...
func (d *DABE) Decrypt(hash hash.Hash, cipher *Cipher, privateKeys map[string]*pbc.Element, gid string) ([]byte, error) {
